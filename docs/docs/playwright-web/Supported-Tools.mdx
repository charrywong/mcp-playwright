...
Ask Playwright to start waiting for a text content to appear on the page. This tool initiates the wait operation but does not wait for its completion.

- **Inputs:**
  - **`text`** *(string)*:
    The exact text content expected to appear on the page.
...
Tag meaningful visible elements on the current page with `data-tag-id` attributes and return a compact list for model processing.
...
- **Response:**
  - **`content`** *(string)*:
    A compact, line-separated list of tagged elements in the format: `data-tag-id:text`.
    Each entry represents a visible and meaningful DOM element (e.g., buttons, inputs, icons with semantic meaning), where:
    - `data-tag-id` is a unique incremental identifier starting from `1`, applied to the element as an attribute like `data-tag-id="1"`.
    - `text` is a short label or description extracted from the element:
      - For form fields: the `placeholder` value if available.
      - For icons: a keyword from known icon classes (e.g., `add`, `edit`, `delete`).
      - For regular elements: the first 5 characters of direct visible text.
...
Get a Playwright locator for an element identified by its data-tag-id attribute.

- **Inputs:**
  - **`id`** *(string)*: The data-tag-id value to locate (e.g. "11" for "11:新建")

- **Response:**
  - **`content`** *(array)*:
    An array containing:
    1. Description of the locator
    2. The actual locator string (e.g. "text=\"新建\"")

    The locator is generated using Playwright's selector generation logic to find the most reliable way to locate the element.

- **Example Usage:**
  After tagging elements with playwright_get_visible_tag, you can get precise locators for specific elements:
  ```
  Input: {"id": "11"}
  Output: ["Tag element locator is:", "text=\"新建\""]
  ```
